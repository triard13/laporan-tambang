...
<head>
    ...
    <title>{{ config('app.name', 'Laporan Tambang') }} - Lupa Kata Sandi</title>
    ...
    <div class="bg-white w-full max-w-[1000px] rounded-[1.5rem] shadow-2xl flex overflow-hidden min-h-[600px]">
        ...
        <div class="w-full md:w-1/2 p-8 md:p-10 lg:p-12 flex flex-col justify-between">
            ...
            <div class="w-full mx-auto flex-grow flex flex-col justify-center">
                <h2 class="text-[1.75rem] font-bold text-gray-900 mb-2">Lupa Kata Sandi?</h2>
                <p class="text-sm text-gray-500 mb-8 font-medium leading-relaxed">
                    Tidak masalah. Cukup beri tahu kami alamat email Anda dan kami akan mengirimi Anda tautan setel ulang kata sandi melalui email.
                </p>
                ...
                <form method="POST" action="{{ route('password.email') }}" class="space-y-4">
                    ...
                    <div>
                        ...
                        <input id="email" type="email" name="email" value="{{ old('email') }}" required autofocus placeholder="mis. admin@mail.com"
                    ...
                    <div class="mt-4 text-center text-xs font-medium">
                        <a href="{{ route('login') }}" class="font-bold text-gray-500 hover:text-gray-700 transition-colors flex items-center justify-center">
        ...
        <div class="hidden md:flex md:w-1/2 bg-[#022c22] relative items-center justify-center overflow-hidden">
            <svg class="absolute inset-0 w-full h-full opacity-20" xmlns="http://www.w3.org/2000/svg" width="100%" height="100%">
                <defs>
                    <pattern id="grid-pattern-forgot" width="40" height="40" patternUnits="userSpaceOnUse">
                ...
                <rect width="100%" height="100%" fill="url(#grid-pattern-forgot)"/>
            </svg>
